...
export class ServiceAuth {
  static canRunService(service: Service, method: Method, deserializedArgs: any): boolean {
    ...
    if (service.serviceMetadata?.auth?.public) return true;

    if (service.serviceMetadata?.auth?.allUsers) return UserAuth.isLoggedIn();

    if (!service.serviceMetadata?.auth?.roles) return UserAuth.hasRole('admin');
